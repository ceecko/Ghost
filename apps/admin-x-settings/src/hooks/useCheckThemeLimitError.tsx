...
interface UseCheckThemeLimitErrorReturn {
    ...
    getDefaultThemeLimitError: (themeName?: string) => Promise<string | null>;
...
export const useCheckThemeLimitError = (): UseCheckThemeLimitErrorReturn => {
    ...
    // Get default error message from the customThemes limiter
    const checkDefaultError = useCallback(async (themeName?: string): Promise<string | null> => {
        if (!limiter?.isLimited('customThemes')) {
            return null;
        }

        try {
            // Use '.' for single theme to force error
            const value = noThemeChangesAllowed ? '.' : (themeName || '.');
            await limiter.errorIfWouldGoOverLimit('customThemes', {value});
            return null; // No error
        } catch (error) {
    ...
    return {
        ...
        getDefaultThemeLimitError: checkDefaultError,
